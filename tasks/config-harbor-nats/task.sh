...
echo "- Your Harbor App Domain [*.${HARBOR_APP_DOMAIN}] should be resolvable to $HARBOR_APP_DOMAIN_IP, routable and accessible via the NSX-T network."
...
check_dns_lookup=$(nslookup api.${HARBOR_APP_DOMAIN})
if [ "$?" != "0" ]; then
  echo "Warning!! Unable to resolve api.${HARBOR_APP_DOMAIN}"
