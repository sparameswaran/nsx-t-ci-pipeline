...
set +e
check_dns_lookup=$(nslookup api.${PAS_SYSTEM_DOMAIN})
if [ "$?" != "0" ]; then
  echo "Warning!! Unable to resolve api.${PAS_SYSTEM_DOMAIN}"
  echo "Proceeding with the assumption that api.${PAS_SYSTEM_DOMAIN} would resolve to a Loadbalancer VIP redirecting to GoRouter ultimately"
  echo ""
else
  resolved_ip=$(echo $check_dns_lookup | grep -A1 ${PAS_SYSTEM_DOMAIN} | awk '{print $NF}' )
  echo "Resolved api.${PAS_SYSTEM_DOMAIN} to $resolved_ip"
fi

check_dns_lookup=$(nslookup sampleapp.${PAS_APPS_DOMAIN})
if [ "$?" != "0" ]; then
  echo "Warning!! Unable to resolve sampleapp.${PAS_APPS_DOMAIN}"
  echo "Proceeding with the assumption that sampleapp.${PAS_APPS_DOMAIN} would resolve to a Loadbalancer VIP redirecting to GoRouter ultimately"
  echo ""
else
  resolved_ip=$(echo $check_dns_lookup | grep -A1 ${PAS_APPS_DOMAIN} | awk '{print $NF}' )
  echo "Resolved sampleapp.${PAS_APPS_DOMAIN} to $resolved_ip"
fi

set -e

export VALIDATE_FOR_PAS=true
